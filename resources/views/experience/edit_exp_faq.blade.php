                      ...
                      <div class="col-xs-6 col-sm-6 col-md-6">
                        <div class="form-group mt-3">
                          ...
                          <div class="form-search form-search-icon-right">
                            <input type="text" id="search_attractionfaq" name="sightname" value="{{ $getfaq[0]->expName }}"
                              class="search_attractionfaqs form-control rounded-3" placeholder="" disabled>
                          </div>
                          <input type="hidden" name='attrid' id="selected_att_id" value="{{ $getfaq[0]->ExperienceId }}"
                            class="form-control rounded-3" required>

                          @foreach($getfaq as $getfaq)

                          <div class="form-group getdata-{{ $getfaq->id }} mt-3">
                            <span class="badge bg-dark edit-btn fa-pull-right mt-3" id="edit-btn" value="0"
                              onclick="edit_expfaq({{ $getfaq->id }})">Edit</span>
                            <strong>Question</strong>
                            <div id="questionmsg-{{ $getfaq->id }}"></div>
                            <textarea type="text" name="question" data-original-value="{{ $getfaq->Question }}"
                              id="question{{ $getfaq->id }}" class="form-control rounded-3"
                              disabled>{{ $getfaq->Question }}</textarea>
                          </div>

                          <div class="form-group">
                            <strong>Answer</strong>
                            <div id="answer-{{ $getfaq->id }}"></div>
                            <textarea type="text" name="answer" data-original-value="{{ $getfaq->Answer }}"
                              id="Answer{{ $getfaq->id }}" class="form-control rounded-3" placeholder=""
                              disabled>{{ $getfaq->Answer }}</textarea>
                          </div>

                          <span id="buttonsContainer-{{ $getfaq->id }}"
                            class="buttons-container-dd d-none mb-3 " data-colid="{{ $getfaq->id }}">
                            <button type="button" value="1"
                              class="reviewField- btn btn-dark save-button px-4 updateexpfaq"
                              data-id="{{ $getfaq->id }}">Save</button>
                            <button type="button" value="2" id="cancel-1"
                              onclick="cancel_expfaq({{ $getfaq->id }})"
                          ...
                          <div class="modal-body">
                            ...
                            <div class="container">
                              ...
                              <span class="mt-3  list-buttons">
                                <button id="savelistfaq" data-id="{{request()->route('id') }}" class="btn btn-dark">Save</button>
                            ...
                            <p id="errorcustfaq"></p>
                              ...
                              <span class="mt-3  custom-faq-buttons">
                                <button id="saveexpfaq" data-id="{{request()->route('id')}}" class="btn btn-dark" disabled>Save</button>
                                <button id="cancel-list" type="button" class="btn btn-dark margin-l" data-bs-dismiss="modal" disabled>cancel</button>
                              </span>
