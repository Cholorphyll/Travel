...
@if (!empty($searchresults))
    @foreach ($searchresults as $searchresult)
        ...
        <li><svg width="32" height="33" viewBox="0 0 32 33" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            </svg><div style=""><span style="text-overflow: ellipsis; white-space: nowrap; overflow: hidden;color: #4A4A4A;font-size: 14px;font-weight: 400;"><a class="custom-link" href="{{ url('lo-'.$searchresult['id'].'-'.strtolower( str_replace(' ', '_', $searchresult['Slug']))  ) }}">{{ $searchresult['value'] }}</a></span></div></li>
    @endforeach
@else
