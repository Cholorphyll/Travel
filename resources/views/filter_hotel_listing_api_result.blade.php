...
<body>
 ...
 <!-- breadcrums -->
      ...
      <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb" class="nav-bar">
      <ol class="breadcrumb">

         @if(!$getcontlink->isEmpty())
          @if($getcontlink[0]->cName !="")
		  <li class="breadcrumb-item active" aria-current="page">
			  <a href="{{ route('explore_continent_list',[$getcontlink[0]->contid,$getcontlink[0]->cName])}}"> {{$getcontlink[0]->cName}}</a>
		  </li>
		  @endif
          <li class="breadcrumb-item active" aria-current="page">
          <a href="{{ route('explore_country_list',[$getcontlink[0]->CountryId,$getcontlink[0]->slug])}}"> {{$getcontlink[0]->Name}}</a>
          </li>
        @endif
    ...
    [2,3,5,7,8,5,3,2,3,6,5,4,7,5,4,3,2]
                    ...
                    <input class="js-range-slider" type="text" data-extra-classes="u-range-slider" data-type="double"
                      ...
                      data-from="200" data-to="750" data-result-min="#rangeSliderExample5MinResult"
        ...
        <div class="sortfilters d-flex justify-content-between">
          ...
          <span class='d-none locationgeo' >{{$locationgeo}}</span>
          <div class="view-map">
        ...
        <hr>
        <h1 class="fs24 fw-500 mb-16">Hotels in {{$lname}}</h1>
        ...
        <span id="Tplocid" class="d-none">{{$Tploc}}</span> 
          ...
          <div class="row mt-3">
            ...
            <div class="col-md-4">
              <div class="card-slider">
                ...
                <div class="hotel-listing-slider slick-initialized slick-slider" style="overflow: hidden; width: 100%; height: 100%;">
                @if ($sresult->image == 1)
    <img src="https://s3-us-west-2.amazonaws.com/s3-travell/hotel-images/{{$sresult->id}}.jpg" alt="" style="max-width: -webkit-fill-available; border-radius: 10px;">
			  @else
				<img src="https://photo.hotellook.com/image_v2/limit/h{{ $sresult->hotelid }}_1/400/229.jpg" alt="" style="max-width: -webkit-fill-available; border-radius: 10px;" style="width: 100%; height: auto; border-radius: 10px;">
        ...
        <!-- <img src="{{ asset('public/images/Hotel lobby.svg') }}" alt="" style="max-width: -webkit-fill-available; border-radius: 10px;"> -->
            ...
            <div class="col-md-5">
              <div class="py-3 px-2 border-toend">
                ...
                <!--        
                <div class="fs20 fs-sm-14 fw-500  mb-2 hotel-link"> <a href=" {{ url('hd-'.'-'.$sresult->id .'-'.strtolower( str_replace(' ', '_', str_replace('#', '!', $sresult->slug))) ) }}" target="_blank">{{$sresult->name}}</a></div> -->

 <?php



      $slugid = $sresult->slugid ;
      $id = $sresult->id;
      $slug = strtolower(str_replace(' ', '_', str_replace('#', '!', $sresult->slug)));
      $name = $sresult->name;

      $url = url("hd-{$slugid}-{$id}-{$slug}?checkin={$checkin}&checkout={$checkout}");
?>

<div class="fs20 fs-sm-14 fw-500 mb-2 hotel-link">
    <a href="{{ $url }}" target="_blank">{{ $name }}</a>
				...
				@endif
                    ...
                    $roomdesc = json_decode($searchresults[0]->Roomdesc);  $a = 1;
                   ...
                   <br>
                    @foreach($value as $key=>$val) 
                     @if($key == 'breakfast' && $val != '')
                        Breakfast included  @if (!$loop->last) , @endif
                      @endif  
                      @if($key == 'Free Cancellation' && $val != '')
                            Free Cancellationif  @if (!$loop->last) , @endif  
                          @endif
                        @if($key == 'freeWifi' && $val != '')
                        Free Wifi  @if (!$loop->last) , @endif
                      @endif 
...
@endif
                        ...
                        @if($key == 'deposit' && $val != '' && $val == 1 )
                          ...
                          Payment directly on the site  @if (!$loop->last) , @endif
                     
                        @endif

                        @if($key == 'deposit' && $val == '' && $val != 1)
                              Reserve Now, Pay Later  @if (!$loop->last) , @endif
                        @endif
            ...
            <div class="question py-3">
              ...
              <?php $hotelName = $item['name'];
                            ...
                            $slug = implode('-', array_slice($nburlParts, 2));
                                  ...
                                  $escapedHotelName = preg_quote($hotelName, '/');
                                  $hotelLink = '<a href="' . route("hotel.detail", [$details]) . '" target="_blank">' . $hotelName . '</a>';
                                  $hotelLinks[$hotelName] = $hotelLink;

                                  $escapedHotelName1 = preg_quote($nb_name, '/');
                                  $hotelLinkNb = '<a href="' . route("hotel_neibor_list", [$lid,$neiborhoodid, $slug]) . '" target="_blank" >' . $nb_name . '</a>';
              ...
              <div class="mb-0">
                <div>
                  ...
                  ?>
                    ...
                    <li>
                      <a href="{{route('hotel.detail',[$url])}}" class=" fs18 " target="_blank">
